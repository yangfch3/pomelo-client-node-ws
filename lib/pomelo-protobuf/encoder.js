...
Encoder.prototype.init = function (protos) {
    this.protos = protos || {};
};

Encoder.prototype.encode = function (route, msg) {
    if (!route || !msg) {
    ...
    //Check msg
    if (!Encoder.prototype.checkMsg(msg, protos)) {
 ...
 */
Encoder.prototype.checkMsg = function (msg, protos) {
    if (!protos || !msg) {
    ...
    for (let name in protos) {
        if (protos.hasOwnProperty(name)) {
            ...
            switch (proto.option) {
                ...
                case 'optional':
                    if (typeof (msg[name]) !== 'undefined') {
                        message = protos.__messages[proto.type] || this.protos['message ' + proto.type];
...
Encoder.prototype.encodeMsg = function (buffer, offset, protos, msg) {
    for (let name in msg) {
        if (msg.hasOwnProperty(name) && !!protos[name]) {
            let proto = protos[name];
...
Encoder.prototype.encodeProp = function (value, type, offset, buffer, protos) {
    let length = 0;
    let message;
    ...
    switch (type) {
        ...
        case 'string':
            ...
            break;
        default:
            message = protos.__messages[type] || this.protos['message ' + type];
            if (!!message) {
 ...
 */
Encoder.prototype.encodeArray = function (array, proto, offset, buffer, protos) {
    let i = 0;
...
Encoder.prototype.writeBytes = function (buffer, offset, bytes) {
...
Encoder.prototype.encodeTag = function (type, tag) {
